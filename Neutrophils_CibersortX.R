...
Neu_ex = data.frame(matrix(NA, nrow = length(universe), ncol = 1))
for (i in namesGEP){
  Neu = filteredGEP[[i]][filteredGEP[[i]][,1] %in% universe,c("GeneSymbol","Neutrophils")] #select gene_names and Neutrophils
  Neu_ex = cbind(Neu_ex,Neu)
}

all(as.character(Neu_ex[,2])==as.character(Neu_ex[,4]))
rownames(Neu_ex) = as.character(Neu_ex[,2])
Neu_ex = Neu_ex[, seq(3, length(Neu_ex), 2)]
colnames(Neu_ex) = c("DCIS","EN","IDC","normal")


Neu_ex[Neu_ex<=1] <- NA
predicted_genes_Neu = apply(Neu_ex, 2, function(x){sum(!is.na(x))})
predicted_genes_Neu
##############
#StErrors
#############
