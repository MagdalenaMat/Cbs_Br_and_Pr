...
for (i in namesGEP){
  Tcell = filteredGEP[[i]][filteredGEP[[i]][,1] %in% universe,c("GeneSymbol","CD8Tcells")] #select gene_names and Tcells
...
for (i in namesStEr){
  Error = StEr[[i]][StEr[[i]][,1] %in% universe,c("GeneSymbol","CD8Tcells")] #select gene_names and Monocytes
...
for(i in names(to_GSEA)){
  ...
  path = "~/Desktop/s7s_Breast/data/2016_2017_NOT_batch_corrected/Tcells_plots/vulcano_repeled/"
...
ImGenes = list()
ImGenes[["Treg_recruitment"]] = c("CCL5","CCR5","CXCL14","CCR8")
ImGenes[["memory_survival"]] = c("IL7R","CXCR4") #,"ITGAE","CD69"
...
ImGenes[["act_sig1"]] = c("CD8A","CD3E") #"CD2",
ImGenes[["act_sig2"]] = c("CD151","FASLG","IFNG","TBX21","BATF3")
ImGenes[["fate"]] = c("SOX4","RAG1","BATF3","CXCL14")
ImGenes[["act"]] = c("CD69","IL19","CD8A","CD8B")#"GZMA","GZMB","GZMK","ICOS","IL18RAP","CD151","CD9","CD32","ZAP70", "CCL4","CCL5",
ImGenes[["CXCR4"]] = "CXCR4"
ImGenes[["migration"]] = c("CTSB","CLIC1","CLIC3","TIMP4")
ImGenes[["inh"]] = c("CTLA4","ITGAE","FOXP3","IL10","TIGIT","IDO1","PDCD1","LAG3","HAVCR2") 
ImGenes[["Treg"]] = c("ITGAE","LAG3") #"CTLA4","IL10","HAVCR2"
ImGenes[["termD"]] = c("TNFRSF4","FASLG")
...
genes = c("Treg_recruitment","memory_survival","term_diff_inh_rud","term_diff_stm_rud","neg_sig","act_sig1","act_sig2",
          "fate","act","CXCR4","migration","inh","Treg","Tcell_activation","termD","diff")
