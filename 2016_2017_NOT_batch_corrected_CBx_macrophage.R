...
library(plotly)
library(magrittr)
setwd("~/Desktop/s7s_Breast/data/2016_2017_NOT_batch_corrected/plots/volcano/")
...
for(i in names(to_GSEA)){
  ...
  path = "~/Desktop/s7s_Breast/data/2016_2017_NOT_batch_corrected/Mac_plots/repeled_vulcano/"
...
ImGenes[["antigen_presentation"]] = c("B2M","HLA.A","HLA.B","HLA.C")
ImGenes[["ER_transport"]] = c("TAP2","CD207","NOS3")
ImGenes[["M2"]] = c("TGFB1", "TGFBI","NFKBIA")
ImGenes[["phagocytosis1"]] = c("TREM2","CD47")
ImGenes[["phagocytosis"]] =c("CLTB", "CTSK","AP1S1","AP1S2","ARF1")
...
ImGenes[["TNF_signaling"]] = c("TNFRSF1A","TRAF5","RIPK1","RBCK1")
ImGenes[["A20"]] = c("TNFAIP3")
genes = c("Foam_cell_diff","M2_polarization","M2_pol2",
          "angiogenesis","E_G_F","ER_stress",
          "antigen_presentation","ER_transport","M2","phagocytosis", 
          "IFN1","IFN2",
          "MMPs","TNF_signaling","A20")

library(reshape2)
for(i in genes[c(1)]){ #,2,4,6,7,8,10,13,14,15
...
path = setwd("~/Desktop/s7s_Breast/data/2016_2017_NOT_batch_corrected/plots/GO_enrichment/")
...
for(i in c("MHC|antigen|present","neutroph","T cell","innate","defense","oxygen|hypoxia",
           "NF-kappa","necrosis factor|necrotic","adhesion","virus")){
  print(i)
  for(j in names(res)){
    print(j)
...
genes = to_plot
names(genes) = c("antigen presentation","neutrophil response","T cell activation","innate immune response","defense response","hypoxia signature",
                 "NF-kappaB signaling","tumor necrosis factor signaling","cell-cell adhesion","virus response")

####-----------------------------------------------------------------------
for(element in names(genes)){
  to_heatmap = Geps[rownames(Geps) %in% genes[[element]],]
  to_heatmap = as.matrix(to_heatmap)
  path = "~/Desktop/s7s_Breast/data/2016_2017_NOT_batch_corrected/Mac_plots/heatmaps/"
  ...
  path = "~/Desktop/s7s_Breast/data/2016_2017_NOT_batch_corrected/Mac_plots/boxplots/"
