...
for(i in names(to_GSEA )[8]){
  to_GSEA[[i]]$Selected <- ifelse(to_GSEA[[i]]$names %in% c("TNFRSF1A","TRAF5","RIPK1","RBCK1","TNFAIP3"), "T", "F")
  #path = "~/Desktop/s7s_Breast/data/2016_2017_NOT_batch_corrected/Mac_plots/"
  ...
  p= ggplot(data.frame(to_GSEA[[i]]), aes(x = FC, y = ml10pv)) +
         ...
         geom_text_repel(
           data = subset(data.frame(to_GSEA[[i]]), to_GSEA[[i]]$names %in% c("TNFRSF1A","TRAF5","RIPK1","RBCK1","TNFAIP3")),
           aes(label = names),
           size = 5,
...
library(reshape2)
for(i in genes[c(2,6,10,12,14,15)]){
