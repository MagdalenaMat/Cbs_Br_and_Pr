#### Magdalena Matusiak
#### reanalysis of data from 19 FEB 2018, includinf EN on March 5th
#### analysis of immune landscape changes in breast cancer progression
#### LCM bulk RNASeq deconvoluted with CibersortX
#### 8 batches data DESeq2 size factor normalized and batch corrected
...
path = "~/Desktop/s7s_Breast/data_after_decon/8_batches_with_EN_batch_corr/Filtered/"
setwd(path)
...
for (i in files){
  nam = mgsub(c("CIBERSORTxGEP_bc_8batches_batch_corr_",".txt_GEPs_Filtered.txt"),c("",""),i)
...
path = "~/Desktop/s7s_Breast/data_after_decon/8_batches_with_EN_batch_corr/StErr/"
...
for (i in files){
  nam = mgsub(c("CIBERSORTxGEP_bc_8batches_batch_corr_",".txt_GEPs_StdErrs.txt"),c("",""),i)
...
#"ATM","PML","DDX5","DROSHA","BAX","FOXO3A"

for(i in names(to_GSEA)){
  plotd = data.frame(LogFC = to_GSEA[[i]][, 2], LZqv = to_GSEA[[i]][, 4], gnames = to_GSEA[[i]][[1]])
  m = plotd[plotd$gnames %in% c("ENO1","P4HB", "CLTB","TAP2","HLA.C",
                                "B2M","GAPDH","HK3", "BAX","C4BPA","TREM2", "APOE",
                                "TREM2", "IDO1","CTSD","UBC","UBB","ANXA2"),]
...
ImGenes = list()
ImGenes[["ER_stress"]] = c("P4HB","UBB","UBC","CTSD","PSMB4","PSMD1","PSMD11","PSMD14","PSMD2","PSME1")
ImGenes[["antigen"]] = c("B2M","HLA.A","HLA.B","HLA.C")
ImGenes[["TAP2"]] = c("TAP2","CD207")
ImGenes[["M2"]] = c("CCL17","IL10")
ImGenes[["phagocytosis"]] = c("TREM2","CLTB","C4BPA","APOE", "CD47")
ImGenes[["phagocytosis2"]] =c("ANXA2","CTSD")
ImGenes[["glykolysis"]] = c("HK3","GAPDH","ENO1")
ImGenes[["IL"]] = c("IL10","IL21R","IL24","IL25")
ImGenes[["IFN"]] = c("NFKBIA","IL1RAP","IL1B")
ImGenes[["apoptosis1"]] = c("ATM","PML","BAX")
ImGenes[["apaptosis2"]] = c("DDX5","DDX41")
ImGenes[["MMPs"]] = c("MMP9","MMP13","MMP11","MMP10","TIMP1","TIMP2")
ImGenes[["oxidatie_stress"]] = c("FOXO3","BAX")
genes = c("ER_stress","antigen","TAP2","M2","phagocytosis","phagocytosis2", "glykolysis","IL","IFN","apoptosis1","apoptosis2","MMPs","oxidatie_stress")
...
path = setwd("~/Desktop/s7s_Breast/data_after_decon/8_batches_with_EN_batch_corr/plots/dotplots/")
...
pathList = geneList$FC
names(pathList) = geneList$ENTREZID
source("https://bioconductor.org/biocLite.R")
biocLite("pathview")

library(pathview)
setwd("~/Desktop/s7s_Breast/data_after_decon/8_batches_with_EN_batch_corr/plots/selected_plots/")
