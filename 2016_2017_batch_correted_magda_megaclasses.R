#macrophages test if same terms are enriched when I changed megaclasses (magda_classes, CD8 CD4 separate)
# B.cells Plasma.cells    CD8Tcells    CD4Tcells     NK.cells    Monocytes Dendritic.cells         Gran
...
path = "~/Desktop/s7s_Breast/data/2016_2017_exp_batch_corrected/2016_2017_group_level_CBx/Filtered/"
setwd(path)
...
for (i in files){
  nam = mgsub(c("CIBERSORTxGEP_2016_2017_BC_",".txt_GEPs_Filtered.txt"),c("",""),i)
...
genecount = gene_count[c(5,3,2,4,1),]
...
colnames(Mono_ex) = c("AVL","DCIS","EN","IDC","normal")
...
path = "~/Desktop/s7s_Breast/data/2016_2017_exp_batch_corrected/2016_2017_group_level_CBx/StErr/"
...
for (i in files){
  nam = mgsub(c("CIBERSORTxGEP_2016_2017_BC_",".txt_GEPs_StdErrs.txt"),c("",""),i)
...
colnames(Mono_ster) = c("AVL","DCIS","EN","IDC","normal")
...
Mono_ex = Mono_ex[,c(5,3,2,4,1)]
Mono_ster = Mono_ster[,c(5,3,2,4,1)]
colnames(Mono_ex)
Geps = Mono_ex
dim(Geps)
...
library(plotly)
library(magrittr)
setwd("~/Desktop/s7s_Breast/data/2016_2017_exp_batch_corrected/2016_2017_group_level_CBx/plots/volcano/")
...
# "ENO1","P4HB", "CLTB","TAP2","HLA.C","MMP9","MMP13","MMP14","MMP1","MMP11", "CASP8",
# "B2M","GAPDH", "BAX","C4BPA", "STAT6", "TLR5",
# "CASP1", "TREM2","CXCR4","MARCO",
...
for(i in names(to_GSEA)){
  ...
  m = plotd[plotd$gnames %in% c("CASP1", "TREM2","CXCR4","MARCO",
                                "P4HB", "CLTB","TAP2","HLA.C","MMP14","MMP1",
                                "UBC","UBB","ANXA2","SPP1","RHOA",
                                "CTSK","VEGFB", "TGFA", "TGFB1"),]
  ...
  p = plot_ly(data = plotd, x = ~LogFC, y = ~LZqv, text = ~ gnames, marker = list(size = 20) ) %>%
    layout(title = i, xaxis = list(title = paste0("log2(FC_",i,")")), yaxis = list(title = "-log10(Zqv)"),
           annotations = a)
  print(p)
  export(p, paste0(i,"_vulcano.png"))
}
...
# "CASP1", "TREM2","CXCR4","MARCO","RHOA",
# "P4HB", "CLTB","TAP2","HLA.C","MMP14","MMP1",
# "UBC","UBB","MARCO","SPP1",
# "CTSK","VEGFB", "TGFA", "TGFB1"

#"TNFRSF1A","TRAF5","RIPK1","RBCK1","TNFAIP3"
for(i in names(to_GSEA)){
  to_GSEA[[i]]$Selected <- ifelse(to_GSEA[[i]]$Zqv < 0.05 , "FDR < 0.05", "Not Sig")
  path = "~/Desktop/s7s_Breast/data/2016_2017_exp_batch_corrected/2016_2017_group_level_CBx/plots/repeled_vulcano/"
...
subd = Geps[rownames(Geps) %in% c("CASP1","MMP9","MMP13","NFKBIA","CD163", "CD68","STAT3","DROSHA","CXCL10","IL12","IL13RA1","ENO1","P4HB","C4BPA"),]
...
ImGenes[["cell_adh"]] = c("FOXP3","GRAP2","HAS2","LILRB4","PDCD1LG2","SNAI2","VCAM1")
ImGenes[["TAMactivation"]] = c("TREM2", "CHIT1", "TLR5") #activation
ImGenes[["M2_pol1"]] = c("TGFB1","IL4R","HCK") #M2 polarizing
ImGenes[["M2_pol2"]] = c("PYCARD","NFKBIA","CASP1")
ImGenes[["M2_rud"]] = c("FN1","NRP2") #M2
ImGenes[["angio1"]] = c("ANXA2","FN1")
ImGenes[["angio2"]] = c("MMP14","VEGFB","VEGFC")
...
ImGenes[["ER_stress"]] = c("P4HB","UBB","UBC","PSMB4","PSMD2","PSME1")
...
ImGenes[["phagocytosis2"]] =c("CLTB", "CTSK","AP1S1","AP1S2","ARF1")
ImGenes[["glykolysis"]] = c("GAPDH","ENO1")
ImGenes[["STAT"]] = c("STAT3","STAT6")
ImGenes[["IFN1"]] = c("B2M","HLA.A","HLA.B","HLA.C")
ImGenes[["IFN2"]] = c("EIF2AK2","IFI35","STAT1","IFIT5")
ImGenes[["apoptosis1"]] = c("ATM","PML","BAX")
ImGenes[["apaptosis2"]] = c("CASP8","TP53I3")
ImGenes[["MMPs"]] = c("TIMP1","TIMP2","MMP14") #"MMP9"
ImGenes[["oxidatie_stress"]] = c("FOXO3","BAX")
ImGenes[["TNF"]] = c("TNFRSF1A","TRAF5","RIPK1","RBCK1")
ImGenes[["A20"]] = c("TNFAIP3")
genes = c("DCIS_phago","M2_pol1","M2_pol2",
          "angio2","E_G_F","ER_stress",
          "antigen","TAP2","M2","phagocytosis2", 
          "IFN1","IFN2",
          "MMPs","TNF","A20")
...
for(i in genes){
  ...
  print(ggplot(long_Imm,aes(gene.names,gene.expression, group = stage, fill =stage)) + 
          ...
          scale_fill_manual(values=c("chartreuse4","red")) + #"dodgerblue3","orange","brown"
...
path = setwd("~/Desktop/s7s_Breast/data/2016_2017_exp_batch_corrected/2016_2017_group_level_CBx/plots/dotplots/")
...
to_plot = list()
for(i in c("phagocytosis|endocytosis","neutrophil","antigen")){
  print(i)
  for(j in names(res)){
    print(j)
...
genes = to_plot
names(genes) = c("endocytosis and phagocytosis","neutrophil response","antigen presentation")

####-----------------------------------------------------------------------
for(element in names(genes)){
  to_heatmap = Geps[rownames(Geps) %in% genes[[element]],]
  to_heatmap = as.matrix(to_heatmap)
  path = "~/Desktop/s7s_Breast/data/2016_2017_exp_batch_corrected/2016_2017_group_level_CBx/plots/GO_heatmaps/"
  ...
  path = "~/Desktop/s7s_Breast/data/2016_2017_exp_batch_corrected/2016_2017_group_level_CBx/plots/GO_barplots/"
